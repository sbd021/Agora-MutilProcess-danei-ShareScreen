...
void CEnterChannelDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_ED_UID, m_edtUid);
}
...
void CEnterChannelDlg::InitCtrls()
{
	...
	m_edtUid.MoveWindow(ClientRect.Width() / 2 - 150, 126, 300, 22, TRUE);
	m_edtUid.SetTip(_T("teacher uid"));
...
void CEnterChannelDlg::DrawClient(CDC *lpDC)
{
	...
	rcText.OffsetRect(0, 48);
	rcText.SetRect(rcClient.Width() / 2 - 180, rcText.top, rcClient.Width() / 2 + 170, rcText.top + 32);
	lpDC->RoundRect(&rcText, CPoint(32, 32));
...
uid_t CEnterChannelDlg::GetUserId()
{
	CString strUid;

	m_edtUid.GetWindowText(strUid);
	if (strUid.Compare(_T("uid")) == 0){
		strUid = _T("");
	}
	return _tcstoul(strUid.GetBuffer(0), NULL, 10);
}
